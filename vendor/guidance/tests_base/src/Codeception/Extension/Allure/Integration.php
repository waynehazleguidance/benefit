...
class Integration extends \Guidance\Tests\Base\Codeception\Extension\BaseAbstract
{
    public const ARGUMENTS_LENGTH = 'arguments_length';
    public const OUTPUT_DIRECTORY_PARAMETER = 'outputDirectory';
    public const DELETE_PREVIOUS_RESULTS_PARAMETER = 'deletePreviousResults';
    public const IGNORED_ANNOTATION_PARAMETER = 'ignoredAnnotations';
    public const DEFAULT_RESULTS_DIRECTORY = 'allure' . DIRECTORY_SEPARATOR . 'results';
    public const DEFAULT_REPORT_DIRECTORY = 'allure-report';
    public const INITIALIZED_PARAMETER = '_initialized';
    ...
    public function _initialize(array $ignoredAnnotations = [])
    {
        ...
        $ignoredAnnotations = $this->tryGetOption(self::IGNORED_ANNOTATION_PARAMETER, []);
        Annotation\AnnotationProvider::addIgnoredAnnotations($ignoredAnnotations);
        $outputDirectory = $this->getOutputDirectory();
        $deletePreviousResults =
            $this->tryGetOption(self::DELETE_PREVIOUS_RESULTS_PARAMETER, false);
        ...
        $this->setOption(self::INITIALIZED_PARAMETER, true);
    }
    ...
    private function getOutputDirectory()
    {
        $outputDirectory = $this->tryGetOption(
            self::OUTPUT_DIRECTORY_PARAMETER,
    ...
    private function prepareOutputDirectory(
        ...
        $initialized = $this->tryGetOption(self::INITIALIZED_PARAMETER, false);
    ...
    public function testEnd(TestEvent $testEvent)
    {
        ...
        if (version_compare(Codecept::VERSION, '3.0.0') > -1 && $testEvent->getTest() instanceof Cest) {
            ...
            foreach ($artifacts as $name => $artifact) {
                YandexAllure::lifecycle()->fire(new AddAttachmentEvent($artifact, $name, null));
            }
        }
    ...
    {
        $argumentsLength = $this->tryGetOption(self::ARGUMENTS_LENGTH, 200);
    ...
    public function getLifecycle()
    {
        if (!isset($this->lifecycle)){
            $this->lifecycle = YandexAllure::lifecycle();
        }
        return $this->lifecycle;
    }

    public function setLifecycle(YandexAllure $lifecycle)
    {
